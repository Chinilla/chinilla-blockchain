...
get_bladebit_url() {
  ...
  GITHUB_BASE_URL="https://github.com/Chinilla/bladebit/releases/download"
...
get_madmax_filename() {
  ...
  CHINILLA_PLOT="chinilla_plot"
  if [ "$KSIZE" = "k34" ]; then
    CHINILLA_PLOT="chinilla_plot_k34"
  fi
  SUFFIX=""
  if [ "$OS" = "macos" ]; then
    if [ "$ARCH" = "arm64" ]; then
  ...
  echo "${CHINILLA_PLOT}-${MADMAX_VER}-${SUFFIX}"
}
...
get_madmax_url() {
  ...
  GITHUB_BASE_URL="https://github.com/Chinilla/chinilla-plotter-madmax/releases/download"
...
DEFAULT_BLADEBIT_VERSION="v2.1.1"
DEFAULT_MADMAX_VERSION="1.0.0"
VERSION=
PLOTTER=$1
shift 1
...
if [ "$PLOTTER" = "bladebit" ]; then
  ...
  mv -f "${madmax_filename}" chinilla_plot
  chmod 755 chinilla_plot
  echo "Successfully installed madmax to $(pwd)/chinilla_plot"
  ...
  mv -f "${madmax_filename}" chinilla_plot_k34
  chmod 755 chinilla_plot_k34
  echo "Successfully installed madmax for k34 to $(pwd)/chinilla_plot_k34"
else
  usage
fi
