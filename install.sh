...
if [ "$PYTHON_MAJOR_VER" -ne "3" ] || [ "$PYTHON_MINOR_VER" -lt "7" ] || [ "$PYTHON_MINOR_VER" -ge "11" ]; then
  echo "Chinilla requires Python version >= 3.7 and  < 3.11.0" >&2
...
python -m pip install --extra-index-url https://pypi.chinilla.net/simple/ miniupnpc==2.2.2
python -m pip install -e ."${EXTRAS}" --extra-index-url https://pypi.chinilla.net/simple/
