...
In the event an emergency fix is required for the release version of Chinilla, members of the Chinilla organization will create a feature branch from the current release branch `1.0.0`.
...
. ./activate
pip install ".[dev]"
black chinilla tests && mypy chinilla tests && flake8 chinilla tests
...
6. Install these linters https://github.com/Chinilla/chinilla-blockchain/tree/main/.github/linters

## Testnets and review environments

With the launch of `1.0.0` we will begin running an official `testnet`.
Prior to the release of `1.1.0` there will be two running test nets. `testnet` and `transaction-beta-testnet`. The `transaction-beta-testnet` testnet will be a beta of the pending 1.1 release, which will enable transactions on the chinilla blockchain.
